...
///          └[BrightPass]─[GaussianBlur]─[Opacity:強さ]───────────[Composite/Blend in1]                 ─ Output
///   Input ──────────────────────────────────────────────────────────────────────────[CrossFade in1]
/// 合成モードは YMM4 標準の Blend 列挙を D2D へ変換（既定 加算）。
/// </summary>
internal sealed class GlowEffectProcessor : VideoEffectProcessorBase
{
    private readonly GlowEffect _item;
    ...
    public GlowEffectProcessor(IGraphicsDevicesAndContext devices, GlowEffect item)
        : base(devices)
    {
        _item = item;
    }
    ...
    {
        if (_brightPass is null || _blur is null || _opacity is null
