...
/// <summary>ライトリーク。光漏れレイヤーを生成し、合成モードで元画像に重ねる。</summary>
internal sealed class LightLeakEffectProcessor : VideoEffectProcessorBase
{
    private readonly LightLeakEffect _item;
    private LightLeakCustomEffect? _leak;
    ...
    public LightLeakEffectProcessor(IGraphicsDevicesAndContext devices, LightLeakEffect item)
        : base(devices)
    {
        _item = item;
    }
    ...
    protected override ID2D1Image? CreateEffect(IGraphicsDevicesAndContext devices)
    {
        ...
        _leak = new LightLeakCustomEffect(devices);
        disposer.Collect(_leak);

        _composite = new D2DEffects.Composite(dc) { InputCount = 2 };
        disposer.Collect(_composite);
        using (var layer = _leak.Output)
    ...
    {
        if (_leak is null || _composite is null || _blend is null || _crossFade is null)
        ...
        _leak.Angle     = (float)(_item.Angle.GetValue(frame, length, fps) * Math.PI / 180.0);
        _leak.Position  = (float)(_item.Position.GetValue(frame, length, fps) / 100.0);
        _leak.Reach     = (float)(_item.Reach.GetValue(frame, length, fps) / 100.0);
        _leak.Intensity = (float)(_item.Intensity.GetValue(frame, length, fps) / 100.0);

        // 3色からグラデーション（4ストップ）を構成してシェーダーへ
        var k1 = _item.Color1; var k2 = _item.Color2; var k3 = _item.Color3;
        _leak.SetStop(0, k1.R / 255f, k1.G / 255f, k1.B / 255f);
        _leak.SetStop(1, k2.R / 255f, k2.G / 255f, k2.B / 255f);
        _leak.SetStop(2, k3.R / 255f, k3.G / 255f, k3.B / 255f);
        _leak.SetStop(3, k3.R / 255f, k3.G / 255f, k3.B / 255f);
